...
describe('conteneur', () => {
  it('ouvre un .xcfg simple', async () => {
    const bytes = new Uint8Array(readFileSync(DIR + '2026-08-20_pages-00.xcfg'))
  ...
  it('ouvre un .xczfg et retrouve son document', async () => {
    const bytes = new Uint8Array(readFileSync(DIR + '2026-08-20_backupwithmedia-00.xczfg'))
  ...
  it('un export sans modification recopie les octets sources', async () => {
    const bytes = new Uint8Array(readFileSync(DIR + 'complète.xcfg'))
    const container = await openContainer(bytes, 'complète.xcfg')
